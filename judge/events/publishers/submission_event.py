"""Publisher for submission events."""
import logging
import uuid
import os
...
logger = logging.getLogger('judge.events.publishers.submission_event')
...
def publish(payload: dict, broker_url: str = None, exchange_name: str = None, channel: str = None, appid: str = None):
    ...
    channel = channel or os.environ.get('EVENT_SUBMISSION_CHANNEL', 'judge.submission')
    ...
    logger.info('Published submission event %s', envelope['id'])
