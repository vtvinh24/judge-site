"""Publisher for hooks execute requests."""
import logging
import uuid
import os
...
logger = logging.getLogger('judge.events.publishers.hooks_execute_request')
...
def publish(payload: dict, broker_url: str = None, exchange_name: str = None, channel: str = None, appid: str = None):
    ...
    channel = channel or os.environ.get('EVENT_HOOKS_EXECUTE_CHANNEL', 'judge.hooks.execute')
    ...
    logger.info('Published hooks execute request %s', envelope['id'])
