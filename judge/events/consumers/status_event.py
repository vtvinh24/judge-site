"""Consumer handler skeleton for status events."""
import logging

from judge.events import schemas as event_schemas

logger = logging.getLogger('judge.events.consumers.status_event')


def handle_event(envelope: dict, message=None):
    channel = envelope.get('channel')
    payload = envelope.get('message')
    logger.info('status_event.handle_event channel=%s id=%s', channel, envelope.get('id'))
